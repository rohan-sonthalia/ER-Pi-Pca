...
def sendData():
    ...
    while(True):
        ...
        if x == b'e':
            switcher = 1.0
            for axis in ("LX", "LY"):
                val = int(map(dataSet[0][axis][1], -1.0 *
                          switcher, 1.0 * switcher, 0, 65535))
            ...
            toSend.append(val)
            val = (dataSet[4]["LEFT"] << 7) | (dataSet[4]["RIGHT"] << 6) | (dataSet[3]["SHARE"][1] << 5) | (dataSet[3]["TOUCHPAD"][1] << 4) | (
                dataSet[3]["OPTIONS"][1] << 3) | (dataSet[3]["PS"][1] << 2) | (dataSet[3]["LB"][1] << 1) | dataSet[3]["RB"][1]
            toSend.append(val)
            val = (dataSet[3]["TRIANGLE"][1] << 7) | (dataSet[3]["CROSS"][1] << 6) | (dataSet[3]["SQUARE"][1] << 5) | (
                dataSet[3]["CIRCLE"][1] << 4) | (dataSet[3]["L1"][1] << 3) | (dataSet[3]["L2"][1] << 2) | (dataSet[3]["R1"][1] << 1) | dataSet[3]["R2"][1]
            toSend.append(val)
            sendToArduino(toSend)
